...
TRASH_CLASSES = {
    ...
    'donut', 'cake', 'bowl', 'fork',
    'knife', 'spoon', 'cell phone', 'book'
}
...
class TrashDetector(Node):
    ...
    def is_new_location(self, x, y):
        """Check if this detection is far enough from previous markers."""
        for (kx, ky) in self.known_locations:
            dist = ((x - kx) ** 2 + (y - ky) ** 2) ** 0.5
            if dist < MIN_DISTANCE_BETWEEN_MARKERS:
                return False
        return True
